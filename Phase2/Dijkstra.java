...
public class Dijkstra {
    ...
    // return path from s to d  (interchange s and d to print in right order)
    public String[] getPath(int d, int s) {
        dijkstra(s, d);
        int counter = 0;
        if (pred[d] == -1) {
            System.out.println(d + " is unreachable from " + s);
        ...
        for (int v = d; v != s; v = pred[v])
            counter++;
        String[] path = new String[counter+1];
        dijkstra(s, d);
        counter = 0;
        for (int v = d; v != s; v = pred[v]) {
            path[counter] = Integer.toString(v);
            counter++;
        }
        path[counter] = String.valueOf(s);
        return path;
    }
